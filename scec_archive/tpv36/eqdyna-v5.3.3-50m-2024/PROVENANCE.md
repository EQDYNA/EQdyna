# TPV36 — EQdyna v5.3.3, 50m, 2024-11-06
...
## Files (43 — cplot, 19 on-fault, 22 off-fault, surface-deformation zip)
...
| `TPV36_EQdyna_v5_3_3_50m_dliu_SCECFinalSurfDisp.zip` | 11,785,251 |

## Published standing

Not in the published metric page for `tpv36` (<https://strike.scec.org/cvws/metric_cvv1_u1/tpv36/metric_cvv1_tpv36_ac_0.html>) — no column carries this submission label. The page does carry other EQdyna column(s), under different labels: `dliu.2` = "EQdyna.v5.3.3.50m.dliu".

## Notes

- **Surface deformation.** TPV36 also takes a surface-deformation file, which is e-mailed rather than uploaded to the CGI and is published at <https://strike.scec.org/cvws/tpv36_surfdef.html>. That page lists user `dliu.3`: max uplift **1.031 m**, max subsidence **0.462 m**, against a group range of 0.986–1.069 m uplift over 8 submissions. Archived here as `TPV36_EQdyna_v5_3_3_50m_dliu_SCECFinalSurfDisp.zip`.

- The public area lists this same submission twice, once under `tpv36` and once under `tpv36_arc`. Every file fetched here is **byte-identical** to the copy in `tpv36_arc/eqdyna-v5.3.3-50m-2024/`; both listings are kept so the archive mirrors what the portal actually offers.
