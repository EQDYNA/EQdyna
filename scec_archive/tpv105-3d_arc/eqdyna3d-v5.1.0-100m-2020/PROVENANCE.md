# TPV105-3D_arc — EQdyna3D v5.1.0, 100m, 2020-09-11
...
credentials and no cookies; all state travels in the POST body.
    ...
    POST https://strike.scec.org/cvws/cgi-bin/cvws.cgi
      o=1005   m=tpv105-3d_arc   mus=dliu.2
...
No public metric page for `tpv105-3d_arc` — <https://strike.scec.org/cvws/metric_cvv1_u1/tpv105-3d_arc/metric_cvv1_tpv105-3d_arc_ac_0.html> returns HTTP 404, so SCEC publishes no cross-code table for this listing.

## Notes

- The public area lists this same submission twice, once under `tpv105-3d_arc` and once under `tpv105-3d`. Every file fetched here is **byte-identical** to the copy in `tpv105-3d/eqdyna3d-v5.1.0-100m-2020/`; both listings are kept so the archive mirrors what the portal actually offers. The published metric standing is recorded on the `tpv105-3d` copy.
