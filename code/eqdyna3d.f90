...
PROGRAM eqdy3d
  ...
  real (kind=8),allocatable,dimension(:,:) :: shl,elestress
  !...fault arrays
...
!  outgm = 'gmt.txt'
  ...
  call mesh4num(fltxyz,dx,xmin,xmax,ymin,ymax,zmin,zmax, &
         numnp,numel,neq,nftnd,master,me,nprocs)
!  write(*,*) 'me=', me, '# of fault node',(nftnd(i),i=1,ntotft),'# of elements',numel

  !*** ALLOCATE ARRAYS ***
  !...nodes' arrays
  allocate(id(ndof,numnp),x(ndof,numnp),stat=alloc_err)
  if(alloc_err /=0) then
    write(*,*) 'me= ',me,'insufficient space to allocate arrays id or x'
  endif
  !...element's arrays
  allocate(ien(8,numel),mat(numel),lm(ned,nen,numel),elestress(nstr,numel),&
...
!  write(*,*) 'before meshgen me=',me    
  call meshgen(fltxyz,dx,xmin,xmax,ymin,ymax,zmin,zmax, &
         numnp,numel,ien,mat,elestress,eleporep,rho,vs, &
         x,neq,id,nr4mpi,er4mpi,n4out,an4nds, &
...
!  endif  
  ...
  allocate(brhs(neq),v(ndof,numnp),d(ndof,numnp),stat=alloc_err)
  if(alloc_err /=0) then
...
!  endif
 ...
 !  write(*,*) 'before driver me=', me,'ndout=',ndout
    call driver(numel,numnp,neq,nftnd,ndout,dout,idhist, &
      x,brhs,d,v,mat,ien,elestress,eleporep,pstrain,id,lm,shl, &
