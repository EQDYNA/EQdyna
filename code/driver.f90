SUBROUTINE driver(numel,numnp,neq,nftnd,ndout,dout,idhist,x,brhs,d,v, &
  mat,ien,elestress,eleporep,pstrain,id,lm,shl,e,pois,mushr,rho,rdampm,rdampk,&
...
!endif  
    ...
    time1 = MPI_WTIME()
    alhs = 0.0
    call qdct2(numel,numnp,neq,shl,ien,x,mat,rdampm,rho,e,lm,alhs,&
...
!	    acc(j,l) = brhs(k)
 ...
 !   !$omp section
      call qdct3(numel,numnp,neq,mat,ien,d,v,rdampk,rdampm,rho,ccosphi,sinphi,&
        mushr,elestress,eleporep,elemass,eleshp,eledet,pstrain,c,lm,brhs,&
        me, master, nprocs)
      time2 = MPI_WTIME()
      ...
      time1 = MPI_WTIME()
   ! !$omp section
      call hrglss(numel,numnp,neq,ien,d,v,rdampk,mat,lm,ss,phi,brhs, me, master, nprocs)
      time2 = MPI_WTIME()
   ...
   deallocate(btmp1)
    ...
    do i=1,ntotft
      if (nftnd(i)>0) then !only nonzero fault node, does faulting. B.D. 10/16/09
        time1 = MPI_WTIME()
        call faulting(i,nftnd(i),numnp,neq,lstr,lstr1,fnms,brhs,d,v,x,id,n4onf, &
