SUBROUTINE qdckd(shg,cc,vl,stress,elresf,constk,zerovl,bdamp,&
  ccosphi,sinphi,porep,mushr,pstrmag)
  use globalvar
  implicit none
  !
  ...
  !...plasticity varables. B.D. 1/5/12
  real (kind=8) :: strmea,taomax, yield, rjust,pstrmea,pstrmag
...
!  compiler!! B.D. 8/7/10
!!$omp parallel do default(shared) private(i,j1,j2,j3)
  do i=1,nen
    j1 = ned * (i - 1) + 1
    j2 = ned * (i - 1) + 2
    j3 = ned * (i - 1) + 3      
...
!!$omp end parallel do
  ...
  taomax = 0.5*(strdev(1)**2+strdev(2)**2+strdev(3)**2) &
    + strdev(4)**2+strdev(5)**2+strdev(6)**2  !second invariant of deviator
