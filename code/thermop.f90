subroutine thermop(nsmp, Tatnode, patnode, ift, nftnd, fric, nt)
  ...
  real(kind=8) :: Tatnode(nftnd), patnode(nftnd), fric(100,nftnd)
  real(kind=8) :: htp, rouctp, lamta, aa0, aa1, aa2, aa3, gama, omega, kapa, tmp, tmp2, ker
  ...
  do i = 1, nftnd
	...
	do j = 1, nt-1
		...
		tmp = tmp + abs(frichis(2,i,j,ift))*frichis(1,i,j,ift)*ker*dt
	enddo 
	Tatnode(i) = tmp/fric_tp_rouc/(pi)**0.5
	fric(51,i) = patnode(i) 
	fric(52,i) = Tatnode(i) + fric_tp_Tini
  enddo 	
	! aa0 = fric_tp_a_th/dxtp/dxtp
	! aa1 = 1.0d0/fric_tp_rouc/fric_tp_h/(2.0d0*pi)**0.5*abs(fric(50,i))*fric(49,i)
	! aa2 = 1.0d0/dt + 2.0d0*fric_tp_a_th/dxtp/dxtp 
	! aa3 = 1.0d0/dt
	
	! Tatnode1(i,numtp) = fric_tp_Tini
	! Tatnode1(i,numtp-1) = aa2*Tatnode1(i,numtp) - aa3*Tatnode(i,numtp) - aa0*fric_tp_Tini - aa1*dexp(-((numtp-1)*dxtp)**2/fric_tp_h**2/2.0d0) - aa0*fric_tp_Tini 
	! Tatnode1(i,numtp-1) = Tatnode1(i,numtp-1)/aa0 	
	! do j = numtp-1, 2, -1 
		! Tatnode1(i,j-1) = aa2*Tatnode1(i,j) - aa3*Tatnode(i,j) - aa0*Tatnode(i,j+1) - aa1*dexp(-((j-1)*dxtp)**2/fric_tp_h**2/2.0d0) - aa0*Tatnode1(i,j+1) 		
		! Tatnode1(i,j-1) = Tatnode1(i,j-1)/aa0
	! enddo 
	! ! The temperature for t+1 at nodes from j = 1, numtp are solved. 
	! do j = 1, numtp
		! if (j == 1) then
			! Tpartt(i,j) = aa0*(Tatnode1(i,j+1) + Tatnode1(i,j+1) - 2.0d0*Tatnode1(i,j))  + aa1*dexp(-((j-1)*dxtp)**2/fric_tp_h**2/2.0d0)
