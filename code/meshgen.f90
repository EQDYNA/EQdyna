subroutine meshgen(fltxyz,dx,xmin,xmax,ymin,ymax,zmin,zmax,&
  nnode1,nelement1,ien,mat,elestress,eleporep,rho,vs,xnode,neq1,id,&
...
! B.D. 1/1/12
  ...
  real (kind=8)::grav=9.8,rhow=1000.,b22=0.44327040,b33=0.50911055, &
    b23=-0.15487679
  real (kind=8),dimension(numat)::rho,vs
  !...working variables
        ...
        do i1=1,ndof
          ...
          if(ix==1) then
            ...
            elseif(iy==2 .and. iz==2 .and. i1==1) then
              er4mpi(1,1) = neq
            elseif(iy==ny .and. iz==nz .and. i1==ndof) then
              nr4mpi(2,1) = nnode
            elseif(iy==ny-1 .and. iz==nz .and. i1==ndof) then
              er4mpi(2,1) = neq
            endif
          elseif(ix==nx) then
            if(iy==1 .and. iz==1 .and. i1==1) then
