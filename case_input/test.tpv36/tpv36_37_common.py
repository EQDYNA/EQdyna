"""
Shared build logic for test.tpv36 and test.tpv37.

The two compsets are identical except for par.tpv and the cohesion-buildup
slope (on_fault_vars index 4). Each case's user_defined_params.py calls
buildParams() with its own tpv id and cohesionSlope.
"""
...
def buildParams(tpv, cohesionSlope):
    par = parameters()
    ...
    for ix, xcoor in enumerate(par.fx):
      for iz, zcoor in enumerate(par.fz):
        ...
        if downDipDistance<=8.0e3:
            par.on_fault_vars[iz,ix,4] = cohesionSlope*(8e3 - downDipDistance)
