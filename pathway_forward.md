...
| 8 | BUG (latent): PML region cascade compares y against x-bound (`y >= xmax0`) in all 4 copies; also `damp` uninitialized when z == zmin0 exactly | 2 | `src/comdampv.f90:29,60`, `src/assembleGlobalKU.f90:149,181` | until fixed | 2026-09-12 | `grep -n "xmax0" src/comdampv.f90` |
