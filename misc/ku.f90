...
subroutine ku
    ...
    do nel = 1, totalNumOfElements
        ...
        if (elemTypeArr(nel)==1 .or. elemTypeArr(nel)>10) then
            call qdckd(eleshp(1,1,nel), mat(nel,1:5), velArr(1:ned,nodeIdElemIdRelation(1:nen,nel)), &
        ...
        elseif (elemTypeArr(nel)==2) then ! PML element. 
            ...
            call elemPMLKu(velArr(1:ned,nodeIdElemIdRelation(1:nen,nel)), efPML, stressArr(stressCompIndexArr(nel)+1:stressCompIndexArr(nel)+21), &
...
subroutine elemPMLKu(vl,f,s,ex,mat1,shg,det,nel)
