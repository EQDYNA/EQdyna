...
* 20260922 v5.15.0 release notes
  ...
  * New - **`git tag` is now gated on CI actually being green for the exact commit being tagged** (`testsys/regression/check_pretag_ci.py`). Rule 15 has asked for this since it was written; nothing enforced it, and v5.13.1 was tagged at a commit that had no completed CI run of its own at tag time.
  * New - **an initial-state equilibrium diagnostic**, `EQDYNA_DUMP_EQUIL`, with its own regression gate. Off by default and a proven no-op when unset.
  * New - four project rules earned this week: 2a (a scripted edit asserts on shape), 15a (the pre-tag CI check above), 20 and 20a (a heavy run is launched detached and polled by its artifact, never by its process).
  * Change - **MPI scaling is now measured from 1 to 32 ranks on both backends** on `test.tpv104`, the standing item-33 deliverable. Fortran reaches 37.33x at 32 ranks (superlinear); the jax-MPI path reaches 5.28x.
  * Known limitation - **the jax-MPI path plateaus at 32 ranks and the cause is measured, not guessed**: element counts are unbalanced across ranks -- four of 32 ranks own ZERO interior elements while the rest own ~19,000 -- and every other rank waits for them, 105 ms of a 172.6 ms step. It is not contention (per-rank spread 1.00x on 31 of 32 ranks) and not exchange cost. Independently confirmed on 4 A100s (48% element spread). Fix in progress.
  * Known limitation - **`test.tpv8` x `python-jax-mpi` has a latent intermittent hang**: ranks sharing one XLA compile cache wedge whenever anything else on the box is also running jax. It passed this release's sweep only because the cache was disabled by hand, and a gate that passes only under a manual workaround is not yet a gate. A durable fix is measured and awaiting an owner decision.
  * Correction - **32-core Fortran BEATS one A100**, 24.71 vs 27.26 ms/step, 1.10x. Break-even is below 32 cores, not the ~37 a previous linear extrapolation gave. Do not quote the extrapolation. Four A100s remain 2.6-3.7x faster than 32 CPU cores.
  * Correction - **the reported "~50 minutes in XLA compile at 32 ranks" was wrong.** Actual compile is 0.42 s at 1 rank and 4.43-4.93 s at every rank count in the full sweep; the hung cell finished in 12.4 s with the cache off. The symptom was the shared-cache wedge above. Three runs over three days were lost to it, each blamed on something else.
  * Note - **gated on a full 31-of-31-cell sweep** (unit, regression and e2e all SUCCESS, exit 0, 3839.6 s) run on the exact tree this tag contains: no source file changed after that sweep. The 9 remaining cells of the 40-cell table are declared unsupported with a recorded reason, not skipped.
  * Note - no physics changed and no reference was regenerated.
  * Known issue - `test.tpv30` is still not gated, and `test.drv.a6` x jax is not gateable on GPU (rupture-arrival flips read 468/458/445 against a budget of 450, inside the GPU's own atomics nondeterminism). Neither budget nor reference was touched.
  * Note - v5.13.0 still has no GitHub Release object; its notes are in pastReleaseNotes.md retroactively.
