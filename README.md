# News in 2026
* 20260923 v5.16.1 release notes
  * Fix - **two concurrent e2e sweeps in one checkout can no longer destroy each other.** `run_e2e.py` and `run_e2e_full.py` take an exclusive non-blocking `flock` on their run tree before any build and before the rotation; a second invocation prints the holder's pid, start time and command line and exits 1 without rotating, without waiting and without falling back to another directory. This is the mechanism rule 21a asked for, and it closes v5.16.0's known limitation: the 2026-09-22 collision that killed a 1500.1 s cell and printed a FAIL indistinguishable from a parity regression can no longer happen silently. Guarded by `testsys/regression/test_e2e_run_tree_lock.py`.
  * New - **a tracked `pre-commit` hook refuses commits made from the main checkout** (where `--git-dir` equals `--git-common-dir`), installed by `install-eqdyna.sh` through `core.hooksPath`. Rules 21/21b were enforced by being read; they are now enforced by git. Commits inside a linked worktree and `git merge --ff-only` are unaffected -- both verified explicitly. Inert until the installer is run in a given clone.
  * New - **the GENERATED fractal fault surface now has a guard of its own**, `testsys/regression/test_fractal_fault_geometry_derivatives.py`: a freshly generated `insertFaultType=2` surface must carry stored `dy/dx` and `dy/dz` that match a recomputation at its own dx and dz (dx != dz deliberately, so a spacing swap cannot pass), without `case.setup` being run. It holds the generated path ~570x tighter than the production gate on the same file, which accepts a single-node error up to 0.0568. Closes v5.16.0's other known limitation (pathway item 71).
  * Fix - **the roughness diagnostic in `scripts/generateFaultInterface` told the truth about only one axis.** It tested `.max()` rather than `abs().max()`, so a surface steep only in the negative direction never warned, and it printed `dSurface_dx` twice while labelling one of them `dSurface_dz`. Diagnostic text only; no generated geometry changes (item 72).
  * Note - patch release: no solver source, no reference, no bound and no rule text changed by the code missions. Gated on unit + regression (exit 0) in an isolated worktree, twice; the e2e sweep was NOT re-run for it and nothing here claims one.
