...
* 20260916 v5.8.6 release notes
  * Fix - the Docker image had never actually built the solver: `ubuntu:22.04`'s `mpich` package depends on `libgfortran5` (the runtime library) but not `gfortran` (the compiler `mpif90` shells out to for every build), so `make` failed with `gfortran: command not found` inside the Dockerfile's own build step. `gfortran` added explicitly to both `Dockerfile` and `install-eqdyna.sh`'s ubuntu branch, which had the identical latent gap (never exposed before because every environment that ran it already had gfortran from something else). A second, related issue found right behind it: `--no-install-recommends` gave the image a STRICTER dependency closure than `install-eqdyna.sh -m ubuntu` itself uses, silently dropping `mpif.h` (a recommended sub-package of `mpich`) and failing one layer further in; the flag is now dropped, since this Dockerfile's whole job is to reproduce that install path, not a stricter one.
  * New - `.github/workflows/publish.yml` gained `workflow_dispatch`, so a build+gate debug iteration no longer requires cutting a release to run (`gh workflow run publish.yml --ref master`); the push step and `:latest` tag stay gated on a real tag push, so a manual run can never reach the registry. `ghcr.io/eqdyna/eqdyna:v5.8.6` and `:latest` are the first images this pipeline has actually published -- v5.8.3/4/5 all failed at one of the layers above; their Release notes are corrected to say so.
  * New - CI parallelized: one sequential job (build+unit+regression+e2e-ci, ~660s) split into `build` + 4 parallel test jobs, 660s -> 303s (-54%) wall clock, same 10-of-24-cell coverage, with a new regression guard (`test_ci_workflow_coverage.py`) proving the workflow's invocations exactly cover `matrix.CI_CELLS`, no gap and no overlap. `actions/checkout`/`actions/setup-python` bumped v2->v4.
  * Fix - two release-guard bugs found auditing v5.8.2's tag-triggered CI run, both exonerating that release rather than reflecting a defect in it: `check_tag_is_annotated` cannot pass under GitHub Actions' own tag-checkout mechanics regardless of the pushed tag's actual annotation status (now falls back to a remote peel check); `check_network_side`'s GitHub Release check had never actually run in CI with no `GH_TOKEN` set (added).
