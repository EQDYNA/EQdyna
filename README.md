...
* 20260924 v5.17.0 release notes
  * New - **test.tpv30 gated** at the ONE 5 s term, dx=500 m, fortran + python-jax (owner decision; PR #5 -> `567e723`). Reference `51b7649` (rule 7): byte-identical to an independent 15:28 Fortran run, sha256 `74196027...`, retiring `642f119`. Bound 1e-10 against an observed jax 1.909216e-14 (~5200x headroom, stated honestly, not as margin to spare). Everyday sweep grew to 23 cells at this point in the series.
  * Fix - **the port never gave a PML node its own elements' gravity** (`e1888e7`): with the bug reverted the cell read 1.202240e+08 Pa; fixed, it closes to 5.2e-14. This is the defect test.tpv30's gate above proves closed.
  * New - **an always-on per-rank profile, on every backend** (PR #6 -> `b348134`). `EQDYNA_PROFILE` parses strictly: unset, `""`, or `"1"` means on, `"0"` means off, and any other value aborts the run with exit code 14 -- no default-on-unrecognised. Bundles the refactor-round-2 retry (keeps reading `Td` in `rsfNucleation`, closing the `4465c17` revert), a numpy hourglass-buffer reuse, and the profile emitter's own guard. Verified byte-identical: 40/40 `frt` files against master's sweep from before this change. Everyday sweep: 23/23.
  * Change - **`python-numpy` is out of every gate** (PR #4 -> `91afba4`): the gated backend axis is now (fortran, python-jax) plus the `test.tpv8` jax-mpi opt-in -- 21 cells, down from 30. The numpy backend's code is untouched and still runs by hand (`--backend numpy`); it simply no longer has to pass for a sweep to go green.
  * New - **rule 25: PR enforcement is live** (PR #3 -> `bf4d451`). A change under `src/`, `testsys/`, or `.github/` reaches master only through a merged, squash-only pull request -- CI's `pr-policy-gate` job and a local `pre-push` hook both refuse a direct push touching those prefixes, with no bypass flag. The repo is now squash-merge-only with delete-on-merge; merge-commit and rebase-merge are disabled because either would break the gate's one-PR-one-commit assumption. Everything else (docs, board, evidence, session logs, rule text, reference artifacts) still pushes directly to master after the local fast suite.
  * Change - **speed**: the everyday sweep fell from 1679.9 s to 289.4 s (`954434c`, 21/21 cells, numpy-out); CI wall fell from 246 s to 180 s (`91afba4`, run 35950407713). Both measured on the same box.
  * Note - **this PR is gated on `python3 testsys/run.py unit regression` only** (green on this branch's head). Per the corrected rule-25/rule-24 ordering (see Known limitations below), the release maintainer runs the full local sweep against this PR's exact post-merge SHA on a clean checkout, commits it as its own evidence commit, runs `check_pretag_ci.py --pre-tag` against that evidence commit, and only then tags -- none of that has happened yet as of this PR.
  * Known limitation - **late-time rupture is gated nowhere.** `test.tpv29`/`test.tpv36`/`test.tpv37` rupture 47-63% of their fault surface after the 5 s gate cutoff (owner-accepted scope reduction, same-day two-term-design retirement; `pathway_forward.md` item 102). Carried forward from `CLAUDE.md`, not new to this release.
  * Known limitation - **jax's `fault` profile bucket is folded into `element`** (`0a96b24`): per-bucket profile granularity for the jax backend is coarser than before for that one bucket.
  * Known limitation - **`compare_nc_files` passes on zero variables compared** (`testsys/compare.py:271-277`): a cell whose comparison silently compares nothing still reads green. Recorded, not fixed, in session log UU.
  * Known limitation - **`check_pretag_ci.py` counts the number of gated cells, not which cells they are, and reads the working tree rather than the committed tree** at pre-tag time: a locally modified, uncommitted matrix could pass the guard.
  * Known limitation - **rule 25's own text is unsatisfiable as written.** It says "a release PR carries rule 24's local-sweep evidence for its exact SHA," but a PR's squash-merge SHA does not exist until the PR merges, so the PR cannot carry evidence keyed to a SHA it has not yet produced. This release follows the corrected order instead -- PR merges as M; M's CI green; the full sweep runs against M's exact SHA on a clean checkout; that evidence is committed as E; `check_pretag_ci.py --pre-tag E` runs; only then is E tagged -- and flags rule 25's text here for the owner to amend rather than editing it in this PR.
