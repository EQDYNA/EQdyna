...
* 20260917 v5.11.0 release notes
  ...
  * Known issue - **`test.tpv30` is not gated: it runs an incomplete SCEC-spec initial-condition method.** The TPV29/30 spec (Part 8) offers two mutually exclusive initial-stress methods; `test.tpv29` uses Method 1 (manually-specified fault traction, no gravity, no boundary tractions needed) and is correct at 500 m, while `test.tpv30` uses Method 2 (traction derived from an explicit-gravity element stress field, which REQUIRES boundary tractions if the mesh boundary can move) -- and those boundary tractions were never implemented. The initial state is therefore not in static equilibrium, the medium deforms from t=0, and the fault's initial shear reads 33.05 MPa before relaxing toward the analytic 27.79 MPa as the rupture arrives. Fix in progress: switch `test.tpv30` to Method 1 (drop the gravity body force for this case, keep the element stress field for the yield calculation only, and apply the manually-specified traction `case.setup` already computes but currently discards). `test.drv.a6` shares the same `C_elastic=0` path and its reference will likely move once fixed (a separate, reviewed commit, rule 7). See `pathway_forward.md` items 39/19(b).
