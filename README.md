...
* 20260916 v5.8.7 release notes
  * Fix - v5.8.6's `verify-published-image` CI job reported a false FAIL: a container built by `docker build` and later run fresh on a SEPARATE job/runner has no working git network/credentials in that context, and `check_tag_is_annotated`'s remote-peel fallback (added for v5.8.2) treated that failed round-trip the same as a confirmed lightweight tag. `check_network_side`'s own, older network checks already handle exactly this class of problem (report UNVERIFIED, not FAIL) -- the new fallback now does the same. v5.8.6's Docker image had, in fact, already published successfully (`docker push` succeeded in its build-and-push job); this release re-runs the full verification clean. No solver changes.
