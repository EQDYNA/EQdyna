...
Verification Project benchmarks (https://strike.scec.org/cvws/). Every
commit is checked at a coarse resolution against frozen reference results;
each case can also be reproduced at its official spec resolution and
duration, which takes hours and is not run automatically.

## Running the benchmarks

Fast check, minutes, safe to run on every change:
...
* `test.tpv29`, `test.tpv30`, `test.tpv36`, and `test.tpv37` are compared
  against their reference at a shorter simulated time than their official
  spec calls for; the physics after that point is not checked automatically.
