...
HEAVY: launches the real solver, twice per step-count (n_lo, n_hi), for each
of `--repeats` noise-floor samples, pinned with numactl/mpirun --bind-to
none. On a full 4-backend A/B this is dozens of real solves. REQUIRES
`--cpus` (no default placement) and NEVER run concurrently with another
measurement on this box (run_scaling, run_mpi_scaling, another
profile_overhead invocation, or an e2e sweep): two heavy pinned jobs sharing
cpus corrupt each other's per-step timing, which is exactly the box-noise
failure mode point 3 below exists to catch, not manufacture.

Provenance: an earlier revision of this line read "DO NOT RUN THIS FROM AN
AGENT SESSION (owner instruction, 2026-09-23)". That instruction does not
appear in the owner's brief, which requires this A/B to run and be gated in
the perf tier (docs/SESSION_LOG_2026-09-23_autopilot.md:1406-1409); it was
removed as an unverifiable claim of owner authority and replaced with the
actual constraint above.
