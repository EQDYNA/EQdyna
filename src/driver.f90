...
subroutine driver
    ...
    do nt = 1, nstep
        ...
        call faulting
        brhs(1:totalNumOfEquations) = brhs(1:totalNumOfEquations)/alhs(1:totalNumOfEquations) ! timeused(7), depreciated
...
subroutine doubleCouplePointSource
    ...
    if (C_dc==1)then
        ...
        if (x(1,i)==100.0.and.x(2,i)==0.0.and.x(3,i)==-2000.)then
            !write(*,*) 'right',i,me
            if (time<0.2)then
                brhs(equationNumIndexArr(locid(i)+2))=brhs(equationNumIndexArr(locid(i)+2))+&
                (1.0e14/0.2/2*time-1.0e14/2/2/3.1415*sin(2*3.1415*time/0.2))
            else
                brhs(equationNumIndexArr(locid(i)+2))=brhs(equationNumIndexArr(locid(i)+2))+1.0e14/2
            endif
        endif
        ...
        if (x(1,i)==0.0.and.x(2,i)==100.0.and.x(3,i)==-2000.)then
            !write(*,*) 'up',i,me    
            if (time<0.2)then
                brhs(equationNumIndexArr(locid(i)+1))=brhs(equationNumIndexArr(locid(i)+1))+&
                (1.0e14/0.2/2*time-1.0e14/2/2/3.1415*sin(2*3.1415*time/0.2))
            else
                brhs(equationNumIndexArr(locid(i)+1))=brhs(equationNumIndexArr(locid(i)+1))+1.0e14/2
            endif
        endif
...
subroutine velDispUpdate
    ...
    do i=1,numnp
        ...
        elseif (dof1(i)==12) then
            ...
            itag=locid(i)
            eqn=equationNumIndexArr(itag+1)
            if (eqn>0) then
    ...
    if (lstr) then
        if((ndout>0).and.(locplt>1)) then
            ...
            do i=1,ndout 
                ...
                elseif(l==3) then
                    k1=equationNumIndexArr(locid(j)+k)
