...
subroutine meshgen
	...
	integer(kind=4)::nnode, nelement, neq0, nxt,nyt,nzt,nx,ny,nz,ix,iy,iz, &
	edgex1,edgey1,edgez1,i,j,k,i1,j1,k1,edgezn
	integer(kind=4)::nxuni,nyuni,nzuni,ift,n1,n2,n3,n4,m1,m2,m3,m4,alloc_err,&
		istatus(MPI_STATUS_SIZE),bndl,bndr,bndf,bndb,bndd,bndu,mex,mey,mez,rlp,rr,ierr,jj,itmp1,&
		ntag,ntags,zz,ivp1,ivp2,ixe,iye,ize,itemp,iye1,nxe,nye,nze,&
		nsx,nsy,nfx,nfz,msnode
	integer(kind=4),dimension(ntotft)::nftnd0,ixfi,izfi,ifs,ifd
	integer(kind=4),allocatable::fltrc(:,:,:,:)
	real(kind=8)::tol,xcoor,ycoor,zcoor,xstep,ystep,zstep,tmp1,tmp2,tmp3,a,b,area,aa1,bb1,cc1,dd1,p1,q1
	...
	real(kind=8)::initialinput(3,91001),xc(3)
	real (kind=8)::omega
