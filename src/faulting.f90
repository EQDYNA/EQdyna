...
subroutine solveRSF(iFault, iFaultNodePair, iFrictionLaw, nsdSlipVector, nsdSliprateVector, nsdTractionVector)
    ...
    do j=1,3 !x,y,z
        ...
        xyzR(j) = nodalForceArr(eqNumIndexArr(eqNumStartIndexLoc(nsmp(1,iFaultNodePair,iFault))+j)) + nodalForceArr(eqNumIndexArr(eqNumStartIndexLoc(nsmp(2,iFaultNodePair,iFault))+j))
    enddo 
    
    do j=1,3 !x,y,z
        ! calculate xyz components of nodal forces that can generate 
        !  the above calculated accelerations for the m-s node pair. 
        nodalForceArr(eqNumIndexArr(eqNumStartIndexLoc(nsmp(1,iFaultNodePair,iFault))+j)) = (-xyzAccVec(j) + xyzR(j)/massMaster)*mr ! Acc.Slave.x/y/z
