...
subroutine netcdf_read_on_fault_eqdyna
    ...
    do ift = 1, ntotft
        do i = 1, nftnd(ift)
            ...
            fric(FRIC_SLOT_SW_FS,  i, ift) = on_fault_vars(ii,jj,1) ! sw_fs
            fric(FRIC_SLOT_SW_FD,  i, ift) = on_fault_vars(ii,jj,2) ! sw_fd
            fric(FRIC_SLOT_SW_D0,  i, ift) = on_fault_vars(ii,jj,3) ! sw_D0
            fric(FRIC_SLOT_RSF_A,  i, ift) = on_fault_vars(ii,jj,4) ! rsf_a
            fric(FRIC_SLOT_RSF_B, i, ift) = on_fault_vars(ii,jj,5) ! rsf_b
            fric(FRIC_SLOT_RSF_DC, i, ift) = on_fault_vars(ii,jj,6) ! rsf_Dc
            fric(FRIC_SLOT_RSF_V0, i, ift) = on_fault_vars(ii,jj,7) ! rsf_v0
            fric(FRIC_SLOT_RSF_R0, i, ift) = on_fault_vars(ii,jj,8) ! rsf_r0
            fric(FRIC_SLOT_RSF_FW, i, ift) = on_fault_vars(ii,jj,9) ! rsf_fw
            fric(FRIC_SLOT_RSF_VW, i, ift) = on_fault_vars(ii,jj,10)! rsf_vw
            fric(FRIC_SLOT_TP_A_HY, i, ift) = on_fault_vars(ii,jj,11)! tp_a_hy
            fric(FRIC_SLOT_TP_A_TH, i, ift) = on_fault_vars(ii,jj,12)! tp_a_th
            fric(FRIC_SLOT_TP_ROUC, i, ift) = on_fault_vars(ii,jj,13)! tp_rouc
            fric(FRIC_SLOT_TP_LAMBDA, i, ift) = on_fault_vars(ii,jj,14)! tp_lambda
            fric(FRIC_SLOT_TP_H, i, ift) = on_fault_vars(ii,jj,15)! tp_h
            fric(FRIC_SLOT_TP_TINI, i, ift) = on_fault_vars(ii,jj,16)! tp_Tini
            fric(FRIC_SLOT_TP_PINI, i, ift) = on_fault_vars(ii,jj,17)! tp_pini
            fric(FRIC_SLOT_CREEP_VMIN, i, ift) = on_fault_vars(ii,jj,18)! creeping slip rate, lower bound
            fric(FRIC_SLOT_INIT_STRIKE_SHEAR,  i, ift) = on_fault_vars(ii,jj,19)! init_strike_shear
            fric(FRIC_SLOT_INIT_NORM,  i, ift) = on_fault_vars(ii,jj,20)! init_norm
            fric(FRIC_SLOT_STATE, i, ift) = on_fault_vars(ii,jj,21)! init_state variable
            fric(FRIC_SLOT_PEAK_SLIPRATE, i, ift) = fric(FRIC_SLOT_CREEP_VMIN, i, ift)         ! peak slip rate
            fric(FRIC_SLOT_VINI_N, i, ift) = 0.0d0                  ! vini_norm
            fric(FRIC_SLOT_VINI_X, i, ift) = fric(FRIC_SLOT_CREEP_VMIN, i, ift)         ! vinix
            fric(FRIC_SLOT_VINI_Z, i, ift) = 0.0d0                  ! viniz
            fric(FRIC_SLOT_TW_T0,  i, ift) = on_fault_vars(ii,jj,22)! tw_t0
            fric(FRIC_SLOT_COHESION,  i, ift) = on_fault_vars(ii,jj,23)! cohesion
            fric(FRIC_SLOT_INIT_DIP_SHEAR, i, ift) = on_fault_vars(ii,jj,24)! init_dip_shear

            fric(FRIC_SLOT_THETA_PC, i, ift) = abs(fric(FRIC_SLOT_INIT_NORM, i, ift))     ! initialize theta_pc as abs(normal stress)
        enddo 
    enddo
