...
do ift = 1, ntotft
    do i = 1, nftnd(ift)
        ...
        tmp = 0.0d0 
        do j = 1, nt-1
            ker = -kapa/(omega - kapa)/(4.0d0*kapa*(nt-j)*dt + 2.0d0*fric(40,i,ift)**2)**0.5
            ker = ker + omega/(omega - kapa)/(4.0d0*omega*(nt-j)*dt + 2.0d0*fric(40,i,ift)**2)**0.5
        ...
        tmp = 0.0d0 
        do j = 1, nt-1
            ker = 1.0d0/(4.0d0*kapa*(nt-j)*dt + 2.0d0*fric(40,i,ift)**2)**0.5
            tmp = tmp + abs(onFaultTPHist(2,i,j,ift))*onFaultTPHist(1,i,j,ift)*ker*dt
        enddo 
