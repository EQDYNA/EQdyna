...
from eqdyna.port import load, run
...
out = run(S, nsteps=nsteps, verbose=True)
t2 = time.time()
print('solve time', t2 - t1, 's for', nsteps or S['nstep'], 'steps')
...
py_out_path = os.path.join(case_dir, 'frt_py.txt')
...
if golden_path:
    ...
    if golden.shape != cols.shape:
        print('SHAPE MISMATCH', golden.shape, cols.shape)
        sys.exit(1)
